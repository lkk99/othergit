...
const c = new Crawler({
    ...
    callback: (error, res, done) => {
        if (error) {
            ...
            const $ = res.$;
            const $searchList = $('.ssr-content');
            const categoryArr = []
            console.log($searchList, 'ppp')
            $searchList.map((index,item)=>{
                console.log($(item).find('._28fPT').text(), 'pppp')
                const name = $(item).find('._28fPT ').text();
                // const href = $(item).find('a').attr('href');
                // const time = $(item).find('.c-color-gray2').text();
                // const content = $(item).find('.content-right_8Zs40').text();
            ...
            fs.createWriteStream("data/3.txt", categoryArr).write(JSON.stringify(categoryArr));
        }
        done();
    }
});
...
//大片首页
c.queue('https://baijiahao.baidu.com/s?id=1751113811530000194&wfr=spider&for=pc');
//微电影 系列电影
