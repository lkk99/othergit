...
(async () => {
    try {
        ...
        await page.goto('https://www.vfinemusic.com/playlist/', { timeout: 0 });
        //截屏
        await page.screenshot({ path: 'vfinemusic.png' });


        //page.$$eval 查询所有符合条件的页面元素(可以进行dom操作)   
        //page.$$(selector) 只能获取元素，不能获取具体内容，返回ElementHandle
        //注意：该方法不要用await，否则后面的page.on('console')方法不会执行
        page.$$eval('.playlist-list-content .playlist-card .playlist-card-body span', (elements) => {
